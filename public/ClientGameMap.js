class ClientGameMap {
    ...
    move() {
        ...
        this.moveData = {
            index: clientPlayer.index, decX: this.decimalPlayerLocationX(), decY: this.decimalPlayerLocationY()
        };
    }
    ...
    drawEnemies(enemies) {
        enemies.forEach(enemy => {
            if (enemy != null) {
                ...
                // textSize(30);
                // text(`X: ${enemy.x} Y: ${enemy.y}`, enemy.x + this.x, enemy.y - 30 + this.y);
            }
    ...
    /**
     * draw blood
     * @param _bloodCoordinates
     */
    drawBlood() {
        fill(255, 0, 0, 50)
        noStroke(); // Remove stroke
        deadEnemies.forEach(deadEnemy => {
            ellipse(this.x + deadEnemy['x'], this.y + deadEnemy['y'], 10, 10);
        })
    }
