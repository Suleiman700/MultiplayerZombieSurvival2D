...
const GunSounds = new GunSoundsC();
...
socket.on("actionsData", function (actionsData) {
    ...
    const firedBullets = actionsData['firedBullets']
    firedBullets.forEach(firedBullet => {
        GunSounds.playFiredBulletSound(currentGun.sounds.firedBullet, firedBullet.startX, firedBullet.startY)
    })
...
socket.once('startGame', function (startDoors) {
    ...
    let mapCoords = {
        rectCoords: clientMap.coords, doorCoords: clientMap.doorCoords,
    }
...
let bloodBuffer; // Off-screen graphics buffer for drawing blood
...
function setup() {
    ...
    wallGuns = [this.topGunPickup = new WallGun(150, -40, new Olympia(clientPlayer.x, clientPlayer.y)),]
...
function draw() {
    ...
    if (!gameActive && !sessionOver) {
        ...
        if ((keyIsDown(82) && !score.reloading) || score.ammoIn == 0 && !score.reloading) {
            ...
            GunSounds.playReloadSound(currentGun.sounds.gunReload)
        }
        ...
        if (mbox.playerInProximity()) {
            if (!mbox.open && !players[clientPlayer.roomIndex].downed) {
...
function changeName() {
    let nameData = {
        name: nameInput.value(), number: clientPlayer.number
    }
